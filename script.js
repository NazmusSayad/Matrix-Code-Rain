...
videoActive: false,
},animation: null,c: null,
ctx: null,lineC: null,
ctx2: null,video: null,WIDTH: window.innerWidth,
HEIGHT: window.innerHeight,COLUMNS: null,
canvii: [],
...
"^",
"~",
],codes: [],createCodeLoop: null,
codesCounter: 0,init: function () {
M.c = document.getElementById("canvas");
M.ctx = M.c.getContext("2d");
M.c.width = M.WIDTH;
M.c.height = M.HEIGHT;M.ctx.shadowBlur = 0;
M.ctx.fillStyle = "#000";
M.ctx.fillRect(0, 0, M.WIDTH, M.HEIGHT);
M.ctx.font = M.font;M.linesC = document.createElement("canvas");
...
M.ctx2 = M.linesC.getContext("2d");M.createLines(M.ctx2);M.COLUMNS = Math.ceil(M.WIDTH / M.settings.COL_WIDTH);for (var i = 0; i < M.COLUMNS; i++) {
M.codes[i] = [];
M.codes[i][0] = {
open: true,
position: { x: 0, y: 0 },
...
}M.loop();M.createCode();window.onresize = function () {
window.cancelAnimationFrame(M.animation);
M.animation = null;
M.ctx.clearRect(0, 0, M.WIDTH, M.HEIGHT);
M.codesCounter = 0;M.ctx2.clearRect(0, 0, M.WIDTH, M.HEIGHT);M.WIDTH = window.innerWidth;
...
var velocity, height, x, y, c, ctx;if (!M.settings.videoActive) {
// slow fade BG colour
...
M.ctx.globalAlpha = 1;
}M.ctx.globalCompositeOperation = "source-over";for (var i = 0; i < M.COLUMNS; i++) {
...
ctx = c.getContext("2d");M.ctx.drawImage(c, x, y, M.settings.COL_WIDTH, height);if (M.codes[i][0].position.y - height < M.HEIGHT) {
...
var column = M.assignColumn();if (column) {
...
var lettersLength = M.letters.length;M.codes[column][0].position = { x: column * M.settings.COL_WIDTH, y: 0 };
M.codes[column][0].velocity = codeVelocity;
M.codes[column][0].strength = M.codes[column][0].velocity / M.settings.VELOCITY_PARAMS.max;for (var i = 1; i <= codeLength; i++) {
var newLetter = M.randomFromInterval(0, lettersLength - 1);
M.codes[column][i] = M.letters[newLetter];
}M.createCanvii(column);M.codesCounter++;
...
var text, fadeStrength;var newCanv = document.createElement("canvas");
var newCtx = newCanv.getContext("2d");newCanv.width = M.settings.COL_WIDTH;
newCanv.height = canvHeight;for (var j = 1; j < codeLen; j++) {
text = M.codes[i][j];
newCtx.globalCompositeOperation = "source-over";
newCtx.font = "30px matrix-code";if (j < 5) {
...
fadeStrength = 1 - fadeStrength;newCtx.shadowOffsetX = 0;
newCtx.shadowOffsetY = 0;
newCtx.shadowBlur = 0;
...
var linesYBlack = 0;
var linesYWhite = 0;ctx.beginPath();ctx.lineWidth = 1;
ctx.strokeStyle = "rgba(0, 0, 0, 0.7)";while (linesYBlack < M.HEIGHT) {
ctx.moveTo(0, linesYBlack);
ctx.lineTo(M.WIDTH, linesYBlack);linesYBlack += 5;
}ctx.lineWidth = 0.15;
ctx.strokeStyle = "rgba(255, 255, 255, 0.7)";while (linesYWhite < M.HEIGHT) {
ctx.moveTo(0, linesYWhite + 1);
ctx.lineTo(M.WIDTH, linesYWhite + 1);linesYWhite += 5;
}ctx.stroke();
},assignColumn: function () {
var randomColumn = M.randomFromInterval(0, M.COLUMNS - 1);if (M.codes[randomColumn][0].open) {
...
window.URL = window.URL || window.webkitURL;navigator.getUserMedia(
{ video: true },
function (localMediaStream) {
M.video = document.createElement("video");
M.video.autoplay = true;
M.video.width = M.WIDTH;
M.video.src = window.URL.createObjectURL(localMediaStream);M.HEIGHT = M.WIDTH * 0.75; // ratio
M.video.height = M.HEIGHT;M.settings.videoActive = true;
},
function (error) {
console.log(error);
}
